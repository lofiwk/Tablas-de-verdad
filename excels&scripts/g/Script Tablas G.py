...
# Función para calcular las salidas según el enunciado "h"
def calcular_salidas(filas):
    S1, S2, S3, S4, S5 = filas
    if S4 == 1:  # Si el interruptor 4 está cerrado
        return [1, 0, 0, 0]  # Habitación 1 encendida
    else:
...
wb = Workbook()
ws = wb.active
ws.title = "Tabla de Verdad Enunciado H"
...
output_path = os.path.join(script_dir, "tabla_verdad_h_con_casos.xlsx")
wb.save(output_path)
