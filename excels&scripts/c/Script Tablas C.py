...
# Función para calcular las salidas según el enunciado "c"
def calcular_salidas(filas):
    S1, S2, S3, S4, S5 = filas
    if sum([S1, S2, S3, S4, S5]) == 1:  # Si solo 1 interruptor está cerrado
        return [0, 1, 0, 0]  # Habitación 2 encendida
    else:
...
wb = Workbook()
ws = wb.active
ws.title = "Tabla de Verdad Enunciado C"
...
output_path = os.path.join(script_dir, "tabla_verdad_c_con_casos.xlsx")
wb.save(output_path)
