...
# Función para calcular las salidas según el enunciado "d"
def calcular_salidas(filas):
    S1, S2, S3, S4, S5 = filas
    if sum([S1, S2, S3, S4, S5]) == 2:  # Si exactamente 2 interruptores están cerrados
        return [0, 0, 0, 0]  # Todas las ampolletas apagadas
    else:
        return [0, 0, 0, 0]  # También apagadas por defecto (aunque no se especifica otro comportamiento)
...
wb = Workbook()
ws = wb.active
ws.title = "Tabla de Verdad Enunciado D"
...
output_path = os.path.join(script_dir, "tabla_verdad_d_con_casos.xlsx")
wb.save(output_path)
