...
# Función para calcular las salidas según el enunciado "f"
def calcular_salidas(filas):
    S1, S2, S3, S4, S5 = filas
    interruptores = [S1, S2, S3, S4, S5]
    if sum(interruptores) == 4:  # Si exactamente 4 interruptores están cerrados
        # Verificar si solo 3 de los 4 cerrados son consecutivos
        consecutivos = "".join(map(str, interruptores)).count("111") > 0 and not "".join(map(str, interruptores)).count("1111") > 0
        if consecutivos:
            return [0, 0, 1, 1]  # Habitaciones 3 y 4 encendidas
...
wb = Workbook()
ws = wb.active
ws.title = "Tabla de Verdad Enunciado F"
...
# Guardar archivo Excel en el mismo directorio que el script
output_path = os.path.join(script_dir, "tabla_verdad_f_con_casos.xlsx")
wb.save(output_path)
