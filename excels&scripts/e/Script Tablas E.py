...
# Función para calcular las salidas según el enunciado "e"
def calcular_salidas(filas):
    ...
    if sum(interruptores) == 4:  # Si exactamente 4 interruptores están cerrados
        # Verificar si los 4 interruptores cerrados son consecutivos
        consecutivos = "".join(map(str, interruptores)).count("1111") > 0
        if consecutivos:
            return [1, 1, 0, 0]  # Habitaciones 1 y 2 encendidas
...
wb = Workbook()
ws = wb.active
ws.title = "Tabla de Verdad Enunciado E"
...
output_path = os.path.join(script_dir, "tabla_verdad_e_con_casos.xlsx")
wb.save(output_path)
