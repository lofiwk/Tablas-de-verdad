...
# Función para calcular las salidas según el enunciado "b"
def calcular_salidas(filas):
    S1, S2, S3, S4, S5 = filas
    if sum([S1, S2, S3, S4, S5]) == 3:  # Si solo 3 interruptores están cerrados
        return [1, 0, 1, 0]  # Habitaciones 1 y 3 encendidas
    else:
...
wb = Workbook()
ws = wb.active
ws.title = "Tabla de Verdad Enunciado B"
...
output_path = os.path.join(script_dir, "tabla_verdad_b_con_casos.xlsx")
wb.save(output_path)
